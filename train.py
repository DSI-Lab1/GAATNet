...
from difussion import GraphDiffusion


# PreTraining settings
parser = argparse.ArgumentParser()
parser.add_argument('--dataset', type=str, default='Computers')
parser.add_argument('--mode', type=str, default='pretrain', help='use adapter or not')
...
difussion = GraphDiffusion(alpha=args.alpha2, diffusion_steps=args.diffusion_steps)
features = difussion.diffuse_features(adj, features)
...
for epoch in range(args.epochs):
    ...
    # save model
    dataset_folder = f"pretrained_{dataset}"
    if not os.path.exists(dataset_folder):
        os.makedirs(dataset_folder)
    if pred_train_f1_score > best_pred_train_f1_score:
        best_pred_train_f1_score = pred_train_f1_score
        save_path = os.path.join(dataset_folder, f"best_model_and_optimizer_{dataset}_AUC{max(auc_values):.2f}_F-score{max(f1_values):.2f}.pt")
        torch.save({
